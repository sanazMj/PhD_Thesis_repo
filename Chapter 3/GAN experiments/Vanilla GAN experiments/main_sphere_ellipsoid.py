...
def my_config():
    ...
    save_path = '/Ryerson/Projects/tumorGAN/GAN_simple_3D/'
    step_report = 100
    name_choice = ''
    softSparsity = False
    diversity = False
    ...
    cuda_device = 0
@ex.automain
def main(space_dim, mode_collapse, cuda_device, beta, d_thresh, pac_num, name_choice, connectivity, dataset_kind,size_wanted, dataset_include_tumor, softSparsity, diversity, step_report, generator_activation, dataset_size, dimension_orig, shape, batch_size, num_epochs, noise_dim,in_channels_dim,
    ...
    if target_points == 7:
        if dataset_kind == 'Matlab':
            data_file = h5py.File('/Ryerson/Projects/tumor_Matlab/Data/Sphere_one_tumor_Feb_28_GCP/' +
                                  'Dataset_' + 'MATLAB_isocenter' + '_len' + str(size_wanted) + '.h5', 'r')
            target_points = target_points_max
            Real_data = data_file['data_with_isocenters']

        else:
            dataset_name = 'Dataset_sphere_full_iso_with_tumor_16_16_16_' + str(size_wanted) +'_7'
    ...
    for epoch in range(num_epochs):
           ...
           # ---------------------
            ...
            if (GAN_model == 'WGAN_GP' and index%itr_critic == 0) or GAN_model != 'WGAN_GP':
                ...
                g_loss, fake_loss_g, fake_pre, d_connect, KLD, d_div = train_generator(discriminator, optimizer_G, real_imgs,
                                                                fake_imgs,
                                                                valid, fake, loss, diversity=diversity,target_points=target_points,connectedFlag=connectivity)
        ...
        if epoch%step_report ==0 or epoch == num_epochs-1:
            ...
            fake_imgs = generator(z)
            dir ='/Ryerson/Projects/tumorGAN/GAN_simple_3D/results/'
